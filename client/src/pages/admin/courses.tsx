...
const CoursesPage = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === 'add') {
        ...
        toast({ title: 'Succès', description: 'Cours modifié avec succès', });
      }
      closeModal();
      fetchCourses();
    } catch (err) {
  ...
  const handleDelete = async (course: Course) => {
    ...
    try {
      ...
      toast({ title: 'Succès', description: 'Cours supprimé' });
      fetchCourses();
    } catch (err) {
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800 text-sm">
                ...
                <tbody>
                  {paginatedCourses.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="text-center py-4 text-gray-400">Aucun cours trouvé</td>
                    </tr>
                  ) : (
        ...
        {modalOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 w-full max-w-md relative z-10 animate-fade-in">
              ...
              <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">{modalMode === 'add' ? 'Ajouter' : 'Modifier'} un cours</h2>
              <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
                ...
                <input
                  ...
                  required
                />
                <label className="text-sm font-semibold text-gray-700 dark:text-gray-200">Professeur</label>
                <select
                  name="professor"
                ...
                <label className="text-sm font-semibold text-gray-700 dark:text-gray-200">Classe</label>
                <input
                  type="text"
                  name="class"
                  placeholder="Classe"
                  ...
                  required
                />
                <label className="text-sm font-semibold text-gray-700 dark:text-gray-200">Horaire</label>
                <input
                  type="text"
                  name="schedule"
                  placeholder="Ex: Lundi 10h-12h Salle 101"
                  className="input input-bordered rounded-lg px-4 py-2"
