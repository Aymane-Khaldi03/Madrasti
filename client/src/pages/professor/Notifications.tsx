        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-4 transition-all duration-300">
          ...
          {pageCount > 1 && (
            <div className="flex justify-center items-center gap-4 mt-4">
              <button className="btn btn-sm rounded-full" onClick={handlePrev} disabled={currentPage === 1} aria-label="Page précédente">Précédent</button>
              <span className="font-semibold">Page {currentPage} / {pageCount}</span>
              <button className="btn btn-sm rounded-full" onClick={handleNext} disabled={currentPage === pageCount} aria-label="Page suivante">Suivant</button>
            </div>
          )}
        </div>
      </main>
